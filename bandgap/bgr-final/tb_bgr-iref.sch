...
N 800 -370 800 -350 {lab=#net13}
N 800 -440 800 -430 {lab=VDD}
N 490 -440 490 -430 {lab=VDD}
N 490 -370 490 -350 {lab=#net14}
...
C {code_shown.sym} 3940 -1280 0 0 {name=s2 only_toplevel=false value="
...
.control
  ...
  * =========================
  * TRANSIENT: Iref health test
  * =========================

  * ---- 0) Pastikan starting condition keras (biar kelihatan startup beneran)
  * uic = skip operating point, start from ICs (lebih brutal)
  * Kalau terlalu brutal dan gagal konvergen, ganti uic -> tanpa uic
  * (Optional) kamu juga bisa set .ic node tertentu di netlist

  * ---- 1) Run transient startup
  tran 1u 5m uic

  * ---- 2) Definisi arus Iref (ubah sesuai arah yg lu mau)
  * Biasanya: Iref = -i(Vmeas) supaya keluar node jadi positif
  let Iref  = i(vmeas)
  let Iref1 = i(vmeas1)

  * ---- 3) Ukur Iref steady-state (ambil nilai mendekati akhir simulasi)
  meas tran Iref_ss  avg Iref  from=4m to=5m
  meas tran Iref1_ss avg Iref1 from=4m to=5m

  * ---- 4) Waktu settle ke 1% (health metric)
  * Def: waktu pertama kali Iref masuk band +/-1% dari steady dan stay (approx)
  let Iref_hi = 1.01*Iref_ss
  let Iref_lo = 0.99*Iref_ss

  meas tran t_settle_hi when Iref = Iref_hi rise=1
  meas tran t_settle_lo when Iref = Iref_lo rise=1

  * ---- 5) Overshoot/undershoot
  meas tran Iref_max max Iref from=0 to=5m
  meas tran Iref_min min Iref from=0 to=5m

  * ---- 6) “Wrong state” detector: apakah sempat nyangkut di ~0?
  * (threshold bisa kamu ubah)
  meas tran t_first_1u when Iref = 1u rise=1

  echo ==== Iref transient health (startup) ====
  print Iref_ss Iref_max Iref_min t_first_1u t_settle_lo t_settle_hi
  print Iref1_ss
...
C {lab_pin.sym} 490 -440 0 0 {name=p26 sig_type=std_logic lab=VDD}
C {lab_pin.sym} 800 -440 0 0 {name=p27 sig_type=std_logic lab=VDD}
C {title.sym} 1560 -80 0 0 {name=l1 author="Dzaki Andriansyah"}
C {lab_pin.sym} 2760 -630 0 0 {name=p4 sig_type=std_logic lab=VSS}
C {ammeter.sym} 490 -400 0 0 {name=Vmeas savecurrent=true spice_ignore=0}
C {ammeter.sym} 800 -400 0 0 {name=Vmeas1 savecurrent=true spice_ignore=0}
